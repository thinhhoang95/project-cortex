...
Important caveat:
- We assume `tv_flights_ordered` (used when `ref_time_str` is present) returns a sufficiently complete set for membership/intersection.
- If the backend ordered endpoint is truncated around the reference time, multi-TV intersection correctness can be wrong (missing flights that reach another TV later).
...
- Keep ordered endpoint for arrival/dwell/sorting metadata if needed.
...
- Flight list is the intersection across all selected TVs (not union).
...
- Focus mode filtering is applied after intersection and keeps rows if any selected TV arrival/window-start falls within the focus window.
...
- Are the APIs on that page time-sensitive (ordered/truncated) in a way that breaks intersection correctness?
