...
function parsePositiveInteger(value: string | null, field: string): [number | null, NextResponse | null] {
  if (value === null) {
    return [
      null,
      NextResponse.json(
...
export async function GET(request: NextRequest) {
  ...
  if (stepNumberError) return stepNumberError;
  ...
  const upstreamUrl = new URL(`${API_BASE_URL}/agent_sol_details`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: 'Failed to fetch agent solution details',
    ...
    console.error('Error fetching agent solution details:', error);
    return NextResponse.json(
      {
        error: 'Failed to fetch agent solution details',
