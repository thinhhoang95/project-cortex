...
export async function GET(request: NextRequest) {
  ...
  const maxRecordsPerMetricRaw = searchParams.get("max_records_per_metric");
  ...
  if (maxRecordsPerMetricRaw !== null) {
    const parsed = Number.parseInt(maxRecordsPerMetricRaw, 10);
    if (!Number.isFinite(parsed) || parsed < 0) {
      return NextResponse.json(
    ...
    params.set("max_records_per_metric", String(parsed));
  }

  try {
    const upstream = await fetch(`${API_BASE_URL}/collapsed_sector_dd_trace?${params.toString()}`, {
      method: "GET",
    ...
    console.error("Error fetching collapsed_sector_dd_trace:", error);
    return NextResponse.json(
      { error: "Failed to fetch collapsed sector DD trace" },
      { status: 500 },
    );
  }
}
