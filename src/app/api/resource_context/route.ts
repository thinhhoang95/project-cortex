...
export async function GET(request: NextRequest) {
  try {
    const upstream = await fetch(`${API_BASE_URL}/resource_context`, {
      method: "GET",
    ...
    if (!upstream.ok) {
      const message = payload?.detail || payload?.error || `Failed to fetch resource context (${upstream.status})`;
    ...
    return NextResponse.json({ error: "Failed to fetch resource context" }, { status: 500 });
  }
}
