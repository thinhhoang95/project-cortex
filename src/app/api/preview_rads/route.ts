...
export async function GET(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(
      `${API_BASE_URL}/preview_rads${params.size > 0 ? `?${params.toString()}` : ""}`,
      {
        method: "GET",
    ...
    if (!upstream.ok) {
      const message =
        payload?.detail || payload?.error || `Failed to fetch RAD previews (${upstream.status})`;
      return NextResponse.json({ error: message }, { status: upstream.status });
    }

    return NextResponse.json(payload);
  } catch (error) {
    console.error("Error fetching preview_rads:", error);
    return NextResponse.json({ error: "Failed to fetch RAD previews" }, { status: 500 });
  }
}
