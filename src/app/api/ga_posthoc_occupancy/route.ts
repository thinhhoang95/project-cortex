...
export async function GET(request: NextRequest) {
  ...
  const pointId = requestUrl.searchParams.get("point_id");
  if (!runId?.trim()) {
  ...
  const upstreamUrl = new URL(`${API_BASE_URL}/ga_posthoc_occupancy`);
  upstreamUrl.searchParams.set("run_id", runId.trim());
  if (pointId?.trim()) upstreamUrl.searchParams.set("point_id", pointId.trim());
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: "Failed to fetch GA posthoc occupancy",
