...
export async function GET(request: NextRequest) {
  ...
  const upstreamUrl = new URL(`${API_BASE_URL}/sa_posthoc_occupancy`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: "Failed to fetch SA posthoc occupancy",
    ...
    console.error("Error fetching SA posthoc occupancy:", error);
    return NextResponse.json(
      {
        error: "Failed to fetch SA posthoc occupancy",
