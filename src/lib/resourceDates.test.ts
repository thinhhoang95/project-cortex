...
describe("resourceDates", () => {
  it("exposes the seeded local manifest date", () => {
    expect(listLocalResourceDates()).toContain("2023-07-16");
    expect(hasLocalResourceSupport("2023-07-16")).toBe(true);
  });

  it("marks local-only and api-only dates correctly", () => {
    const availability = mergeResourceAvailability(
      {
        selected_date: "2023-07-16",
        available_dates: ["2023-07-16", "2023-07-29"],
    ...
    expect(availability.find((entry) => entry.date === "2023-07-16")?.status).toBe("ready");
  ...
  it("confirms readiness only when both backend and local manifest support the date", () => {
    const context = {
      selected_date: "2023-07-16",
      available_dates: ["2023-07-16"],
      manifest_path: "/srv/resource_manifest.json",
      generation: 1,
      resolved_paths: {},
    };

    expect(isResourceDateReady("2023-07-16", context)).toBe(true);
