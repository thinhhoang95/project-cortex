...
export default function RegulationCanvas() {
  ...
  const updateTrafficVolumeLayers = useCallback(() => {
    ...
    const applyLayerState = (layerId: string, filter: any, overrideVisibility?: "visible" | "none") => {
      if (!map.getLayer(layerId)) return;
      const visibility = shouldHideTrafficVolumes ? "none" : (overrideVisibility ?? "visible");
    ...
    applyLayerState("sector-slack", baseFilter, slackMode === 'off' ? "none" : "visible");
  ...
  }, [flLowerBound, flUpperBound, highlightedTrafficVolume, hoveredTrafficVolume, getActiveHotspots, slackMode]);
  ...
  // Keep traffic volume layers in sync with FL range, highlight, and hotspot state
  useEffect(() => {
    updateTrafficVolumeLayers();
