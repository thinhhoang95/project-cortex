...
describe("useSimStore VPF flow extractor state", () => {
  ...
  it("stores VPF memberships while preserving a primary community fallback", () => {
    useSimStore.getState().setFlowCommunities(
      ...
      { F1: [0, 1], F2: [0], F3: [1] },
      {
        "0": { secondaryLabel: "TVB", secondaryWindowLabel: "09:00-10:00", proxyScore: 5.25 },
        "1": { secondaryLabel: "TVC", secondaryWindowLabel: "10:00-11:00", proxyScore: 4.5 },
      },
    ...
    expect(useSimStore.getState().flowGroupMetadata?.["0"]?.secondaryLabel).toBe("TVB");
